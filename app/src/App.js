...
class  App extends React.Component {
  ...
  componentDidMount(){
    axios.get("http://3.138.135.4:31111/api/")
    .then((res)=>{
      this.setState({
        users:res.data,
        id:0,
  ...
  submit(evenet,id){
    ...
    if(id===0){
      axios.post("http://3.138.135.4:31111/api/",{
      ...
      axios.put("http://3.138.135.4:31111/api/",{
        id:id,
        name:this.state.name,
        email:this.state.email,
        password:this.state.password
  ...
  delete(id){
    axios.delete("http://3.138.135.4:31111/api/"+id)
    .then(()=>{
      this.componentDidMount();
    })
  }
  edit(id){
    axios.get("http://3.138.135.4:31111/api/"+id)
    .then((res)=>{
      this.setState({
        id:res.data.id,
        name:res.data.name,
